...
function Select({ label, value, onChange, children }: { label: string; value: string; onChange: (value: string) => void; children: ReactNode }) {
  return (
    <label className="flex min-w-[145px] flex-col gap-1 text-[10px] font-bold uppercase tracking-[.12em] text-slate-500">
      {label}
      <select value={value} onChange={event => onChange(event.target.value)} className="h-11 rounded-lg border border-slate-200 bg-white px-3 text-sm font-medium normal-case tracking-normal text-[#12334a] outline-none focus:border-[#16877f] focus:ring-2 focus:ring-[#16877f]/10">
        {children}
      </select>
    </label>
  );
