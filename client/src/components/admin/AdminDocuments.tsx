...
function emptyForm(view: View): FormState {
  return { partyName: "", contact: "", notes: "", dueDate: "", status: "draft", items: [emptyItem()] };
}

function buttonClass(tone: "primary" | "quiet" | "danger" = "quiet") {
  return tone === "primary"
    ? "rounded-lg bg-[#12334a] px-4 py-2.5 text-xs font-bold uppercase tracking-[.1em] text-white transition hover:bg-[#0c283b] focus:outline-none focus:ring-2 focus:ring-[#16877f]/40 disabled:cursor-not-allowed disabled:opacity-50"
    : tone === "danger"
      ? "rounded-lg border border-[#ebc4be] px-3 py-2 text-xs font-bold uppercase tracking-[.08em] text-[#a44539] transition hover:bg-[#fff0ee] focus:outline-none focus:ring-2 focus:ring-[#bd5d4e]/30"
      : "rounded-lg border border-slate-300 px-3 py-2 text-xs font-bold uppercase tracking-[.08em] text-[#12334a] transition hover:border-[#16877f] hover:bg-[#f1f8f7] focus:outline-none focus:ring-2 focus:ring-[#16877f]/30 disabled:cursor-not-allowed disabled:opacity-50";
}
...
function DocumentForm({
  ...
  const previewTotal = form.items.reduce((sum, item) => sum + (Number(item.quantity) || 0) * (Number(item.unitPrice) || 0), 0);
      ...
      <div className="mt-5">
        ...
        <div className="space-y-2">
          {form.items.map((item, index) => <div key={index} className="grid gap-2 rounded-lg border border-slate-200 bg-white p-3 sm:grid-cols-[minmax(0,1fr)_90px_120px_auto] sm:items-end">
            ...
            <button type="button" onClick={() => setForm({ ...form, items: form.items.length === 1 ? [emptyItem()] : form.items.filter((_, itemIndex) => itemIndex !== index) })} className={buttonClass("danger")} aria-label={`Remove line ${index + 1}`}>Remove</button>
          </div>)}
        </div>
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-4"><span className="text-xs text-slate-500">Client preview total · server total is authoritative after save</span><strong className="font-serif text-xl text-[#12334a]">{money(previewTotal)}</strong></div>
      </div>
      <div className="mt-5 flex flex-col-reverse justify-end gap-2 sm:flex-row"><button type="button" onClick={onCancel} className={buttonClass()} disabled={saving}>Cancel</button><button type="submit" className={buttonClass("primary")} disabled={saving}>{saving ? "Saving…" : editing ? "Save changes" : `Save ${isQuotation ? "quotation" : "purchase order"}`}</button></div>
    </form>
  );
}
...
export function AdminDocuments({ view }: { view: View }) {
  ...
  const openEdit = (document: AdminDocument) => {
    setEditing(document); setForm({ partyName: document.partyName, contact: document.contact || "", notes: document.notes || "", dueDate: document.dueDate || "", status: document.status, items: document.items.length ? document.items.map((item) => ({ ...item })) : [emptyItem()] }); setError("");
  };
  const save = async (event: FormEvent) => {
    event.preventDefault(); if (!form) return;
    setSaving(true); setError("");
    const body = { partyName: form.partyName.trim(), contact: form.contact.trim(), notes: form.notes.trim(), dueDate: form.dueDate || null, items: form.items.map((item) => ({ description: item.description.trim(), quantity: Number(item.quantity), unitPrice: Number(item.unitPrice) })), status: form.status };
    try {
  ...
  const updateStatus = async (document: AdminDocument, status: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/${endpoint}/${encodeURIComponent(String(document.id))}`, { method: "PATCH", credentials: "same-origin", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ partyName: document.partyName, contact: document.contact || "", notes: document.notes || "", dueDate: document.dueDate, items: document.items, status }) });
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-[0_8px_25px_rgba(18,51,74,.05)]">
      ...
      {loading ? <div className="space-y-3 pt-4" aria-label="Loading documents"><div className="h-16 animate-pulse rounded-lg bg-slate-100" /><div className="h-16 animate-pulse rounded-lg bg-slate-100" /><div className="h-16 animate-pulse rounded-lg bg-slate-100" /></div> : error && !documents.length ? <div className="px-5 py-14 text-center text-sm text-slate-500">Documents could not be loaded. Retry to see current records.</div> :
        ...
        <div className="divide-y divide-slate-200">{filtered.map((document) => <article key={document.id} className="grid gap-4 py-4 md:grid-cols-[minmax(0,1.2fr)_minmax(0,.8fr)_minmax(0,.8fr)_auto] md:items-center">
          ...
          <div className="flex flex-wrap gap-2 md:justify-end"><button type="button" onClick={() => { if (!printDocument(document)) setError("The browser blocked the print window. Allow pop-ups and try again."); }} className={buttonClass()}>Print</button><button type="button" onClick={() => openEdit(document)} className={buttonClass()}>Edit</button><select aria-label={`Update status for ${document.number}`} value={document.status} disabled={updatingId === document.id} onChange={(event) => void updateStatus(document, event.target.value)} className="h-9 rounded-lg border border-slate-300 bg-white px-2 text-xs font-bold text-[#12334a] outline-none focus:border-[#16877f]">{statuses.map((status) => <option key={status} value={status}>{statusLabel(status)}</option>)}</select></div>
        </article>)}</div>}
    </section>
  </div>;
}
