...
import { quotationTotals } from "./quotation-totals.js";
...
const documentInput = z.object({
  ...
  discount: z.number().finite().min(0).max(100000000)
    .refine(value => Math.abs(value * 100 - Math.round(value * 100)) < 0.000001, "Use at most two decimal places")
    .default(0),
...
const projection = sql`id, number, kind, party_name AS "partyName",
  contact, notes, due_date AS "dueDate", status, items, discount::float8 AS discount, total::float8 AS total,
  created_at AS "createdAt", updated_at AS "updatedAt"`;

export function validated(input: unknown, kind: Kind) {
  const parsed = documentInput.safeParse(input);
  if (!parsed.success || !statuses[kind].includes(parsed.data.status)) return null;
  const data = parsed.data;
  const { subtotal, total } = quotationTotals(data.items, data.discount);
  if (kind !== "quotation" && data.discount !== 0) return null;
  if (!Number.isFinite(total) || total < 0 || total > 100000000 || data.discount > subtotal) return null;
  return { ...data, total };
}
      ...
      try {
        const document = securityRows(await getDb().execute(sql`
          INSERT INTO admin_documents
             (kind, number, party_name, contact, notes, due_date, status, items, discount, total, created_by)
          VALUES
            (${kind}, ${prefix} || to_char(now(), 'YYYY') || '-' ||
              ...
              ${input.partyName}, ${input.contact}, ${input.notes}, ${input.dueDate ?? null},
               ${input.status}, ${JSON.stringify(input.items)}::jsonb, ${input.discount}, ${input.total},
              ${res.locals.admin.id})
    ...
    app.patch(`${path}/:id`, async (req, res) => {
      ...
      try {
        ...
        const input = validated({
          ...
          discount: patch.data.discount ?? existing.discount,
        }, kind);
        if (!input) return res.status(400).json({ message: "Check the party, date, status and line items." });
        const updated = securityRows(await getDb().execute(sql`
          UPDATE admin_documents SET party_name = ${input.partyName},
            ...
            due_date = ${input.dueDate ?? null}, status = ${input.status},
             items = ${JSON.stringify(input.items)}::jsonb, discount = ${input.discount}, total = ${input.total}, updated_at = now()
